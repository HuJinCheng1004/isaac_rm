...
* **collision** : 底座到目标的水平距离 < ``collision_distance``。
...
from .observations import _to_torch, get_bbox3d_state
...
class TaskOutcome(ManagerTermBase):
    ...
    def __init__(self, cfg: TerminationTermCfg, env: "ManagerBasedRLEnv"):
        ...
        self.d_target: float = float(p.get("d_target", 0.8))
        self.center_tol: float = float(p.get("center_tol", 0.15))
        self.dist_tol: float = float(p.get("dist_tol", 0.2))
        self.collision_distance: float = float(p.get("collision_distance", 0.45))
        self.success_dwell: int = int(p.get("success_dwell", 10))
        self.lost_dwell: int = int(p.get("lost_dwell", 10))
    ...
    def __call__(
        ...
        base_xy = _to_torch(robot.data.root_pos_w)[:, 0:2]
        tgt_xy = _to_torch(target.data.root_pos_w)[:, 0:2]
        dist = torch.linalg.norm(base_xy - tgt_xy, dim=-1)
        ...
        # 为奖励项缓存标志（如果两者都存在，碰撞优先于成功）
        failure = collision | lost
